...
def main(args):
    # 参数验证
    if not args.load_splits_from and not args.data_file:
        raise ValueError("Either --data_file or --load_splits_from must be provided")

    print("=" * 80)
    print("ImmuneApp Mode 1 (HLA-only) - End-to-End Training (Enhanced)")
    print("=" * 80)
    if args.load_splits_from:
        print(f"  Mode: Loading from Mode 2")
        print(f"  Mode 2 output: {args.load_splits_from}")
    else:
    ...
    # 检查是否从Mode 2加载数据
    if args.load_splits_from:
        print(f"\n📂 Loading splits from Mode 2: {args.load_splits_from}")

        splits_dir = Path(args.load_splits_from) / 'data_splits'
        if not splits_dir.exists():
            raise FileNotFoundError(f"Data splits not found: {splits_dir}")
        ...
        # 重命名列（如果需要）
        if 'MHC_Restriction_Name' in train_df.columns:
        ...
        print(f"✓ Loaded Mode 2 splits:")
        print(f"  Train: {len(train_df):,}")
        print(f"  Val:   {len(val_df):,}")
        print(f"  Test:  {len(test_df):,}")
        print(f"  ⏭️  Skipping filtering/splitting")
...
if __name__ == "__main__":
    ...
    data_group.add_argument('--data_file', type=str, required=False,
                            help='Path to data file (TSV). Not required if --load_splits_from is used.')
    data_group.add_argument('--load_splits_from', type=str, default=None,
                            help='Load train/val/test splits from Mode 2 output directory')
