...
def generate_test_datasets(test_df, mode1_dir, negative_ratio, min_samples=1, use_negative_cache=False, cache_dir=None, mode2_output_dir=None):
    """
    ...
    if use_negative_cache and cache_dir:
        # 从 NegativeSampleCache pkl 直接加载
        import glob, pickle as _pkl
        print(f"  Using negative cache: {cache_dir}")
        test_pkls = sorted(glob.glob(str(Path(cache_dir) / '*_test.pkl')))
        if not test_pkls:
            print(f"  ⚠️  No *_test.pkl found in {cache_dir}, falling back to realtime generation")
            use_negative_cache = False
        else:
            # 优先匹配包含 mode2_output_dir 目录名的 pkl，避免误取其他实验的缓存
            if cache_dir and mode2_output_dir:
                mode2_stem = Path(mode2_output_dir).name  # e.g. "0227_mode2_proteome"
                matched = [p for p in test_pkls if mode2_stem in Path(p).name]
                test_pkl = matched[-1] if matched else test_pkls[-1]
            else:
                test_pkl = test_pkls[-1]
            print(f"  ✓ Loading: {Path(test_pkl).name}")
            with open(test_pkl, 'rb') as _f:
                cached_test = _pkl.load(_f)
            print(f"  ✓ Loaded {len(cached_test)} tasks from cache")
            for task_id, task_df in cached_test.items():
    ...
    if not use_negative_cache:
        ...
        for task_id, task in all_tasks.items():
            ...
            neg_df = pd.DataFrame({'peptide': neg_peptides, 'hla': task.hla, 'label': 0})
...
def compute_metrics_and_report(all_predictions, all_tasks, output_dir):
    ...
    with open(summary_file, 'w') as f:
        ...
        valid_f1s = df['f1'].dropna()
        if len(valid_f1s) > 0:
            f.write(f"Mean F1:       {valid_f1s.mean():.4f} ± {valid_f1s.std():.4f}\n")
...
def main():
    ...
    parser.add_argument('--min_samples', type=int, default=1,
                        help='Min samples per task (default: 1)')
    parser.add_argument('--use_negative_cache', action='store_true',
                        help='Load negatives from NegativeSampleCache pkl')
    parser.add_argument('--cache_dir', type=str, default=None,
                        help='NegativeSampleCache directory')
    parser.add_argument('--mode2_output_dir', type=str, default=None,
                        help='Mode 2 output dir (unused, for interface consistency)')
    ...
    filtered_test_datasets, task_manager, all_tasks = generate_test_datasets(
        ...
        use_negative_cache=args.use_negative_cache,
        cache_dir=args.cache_dir,
        mode2_output_dir=args.mode2_output_dir
    )
