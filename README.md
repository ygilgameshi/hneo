...
### 人类基准测试（IEDB 留出测试集，975 个 HLA×Tissue 任务）

| 模型 | AUROC | σ(AUROC) | AUPRC | F1 |
|:---|:---:|:---:|:---:|:---:|
| **HistoNeo w/ Tissue** | **0.9942** | **0.0257** | **0.9588** | **0.9209** |
| MHCflurry 2.0 | 0.9728 | 0.0412 | 0.9148 | 0.8264 |
| HistoNeo w/o Tissue | 0.9778 | 0.0347 | 0.8814 | 0.7449 |
| NetMHCpan 4.2c | 0.9645 | 0.0477 | 0.8968 | 0.7629 |
| MixMHCpred 3.0 | 0.9598 | 0.0520 | 0.8819 | 0.7598 |

HistoNeo w/ Tissue 在全部四项指标上均领先。与最强基线（MHCflurry 2.0）相比：**AUROC +2.2%**、**AUPRC +4.8%**、**F1 +23.6%**。与 w/o Tissue 相比：AUROC 方差降低 **25.9%**，证明组织条件化能够在多样化的 HLA 等位基因间稳定预测结果。

一个反直觉的发现：组织条件化带来的增益在**训练样本稀少的罕见任务上最为显著**，说明组织上下文在数据匮乏时能够提供有效的归纳偏置。

### 跨物种验证（小鼠，6 个等位基因）

| 模型 | AUROC |
|:---|:---:|
| **HistoNeo w/ Tissue** | **0.9540** |
| HistoNeo w/o Tissue | 0.8750 |
| NetMHCpan 4.2c | 0.8446 |
| MixMHCpred 3.0 | 0.8410 |
| MHCflurry 2.0 | 0.7230 |

跨物种结果证实了组织信号的生物学普遍性（相比 w/o Tissue，AUROC +9.0%）。

### 消融实验

| 变体 | AUROC | 说明 |
|:---|:---:|:---|
| **完整模型** | **0.9942** | GNN + FiLM + 自适应采样 |
| −GNN | 0.9922 | 用原始节点嵌入替代 GNN 传播 |
| −FiLM | 0.9912 | 用 Concat + Linear 融合替代 FiLM |
| −Sampling | ~0.50 | 禁用自适应任务均衡采样 → 训练崩溃 |

−Sampling 的结果是一项**机制性发现**：在去除任务均衡采样后，模型退化为接近随机的分类器（AUROC ≈ 0.50），证明自适应采样对于在高度不均衡的任务分布上有效学习是不可缺少的。
