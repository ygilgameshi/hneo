...
class PerTaskEvaluator:
    ...
    def _generate_summary(self, df):
        ...
        if 'hla' in df.columns:
            ...
            summary['by_hla'] = {
                hla: {
                    'mean_auroc': float(hla_stats['auroc']['mean'][hla]),
                    'n_tasks': int(hla_stats['auroc']['count'][hla]),
                    'mean_accuracy': float(hla_stats['accuracy']['mean'][hla]),
                    'total_samples': int(hla_stats['n_samples']['sum'][hla])
                }
                for hla in hla_stats['auroc']['mean'].keys()
            }
        ...
        if 'tissue' in df.columns:
            ...
            summary['by_tissue'] = {
                tissue: {
                    'mean_auroc': float(tissue_stats['auroc']['mean'][tissue]),
                    'n_tasks': int(tissue_stats['auroc']['count'][tissue]),
                    'mean_accuracy': float(tissue_stats['accuracy']['mean'][tissue]),
                    'total_samples': int(tissue_stats['n_samples']['sum'][tissue])
                }
                for tissue in tissue_stats['auroc']['mean'].keys()
            }
        ...
        summary['by_sample_size'] = {
            str(bin_name): {
                'mean_auroc': float(sample_stats['auroc']['mean'][bin_name]),
                'n_tasks': int(sample_stats['auroc']['count'][bin_name])
            }
            for bin_name in sample_stats['auroc']['mean'].keys()
        }
