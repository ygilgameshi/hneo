...
from data.task_definition import TaskManager
from data.mode_aware_dataset import create_mode_aware_dataloaders
from models.mode_aware_model import ImmuneAppModel
from graph.mode_aware_task_graph import ModeAwareTaskGraphBuilder, TaskGraphWrapper
...
class StandardTrainer:
    ...
    def train_epoch(self, train_loader, graph_wrapper):
        """训练一个epoch"""
        self.model.train()
        epoch_losses = []
...
def train_model(
    ...
    **kwargs
    ...
    train_loader, val_loader, test_loader = create_mode_aware_dataloaders(
        task_datasets,
        val_task_datasets,
        test_task_datasets,
        task_manager,
    ...
    if mode_config.mode == TrainingMode.HLA_TISSUE:
        # 从task_manager计算unique tissues数量
        ...
        print(f"  Mode 2: {model_kwargs['n_tissues']} tissues")
    ...
    if mode_config.use_maml:
        ...
        trainer = StandardTrainer(model, optimizer, criterion, device)
    ...
    # 导入per-task评估器
    from utils.per_task_evaluator import evaluate_per_task_performance
    ...
    # 将per-task结果添加到history中（可选）
    results['per_task_mean_auroc'] = float(per_task_summary['overall']['mean_auroc'])
    results['per_task_median_auroc'] = float(per_task_summary['overall']['median_auroc'])
    results['n_tasks'] = int(per_task_summary['overall']['n_tasks'])
