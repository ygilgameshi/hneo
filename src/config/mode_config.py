...
class ModeConfig:
    ...
    use_cross_hla_negatives: bool = True
    use_tissue_aware_negatives: bool = False  # Mode 2专用
    ...
    def __post_init__(self):
        ...
        if self.mode == TrainingMode.HLA_ONLY:
            # Mode 1不使用tissue信息
            self.use_tissue_aware_negatives = False
...
def create_mode2_config(**kwargs) -> ModeConfig:
    ...
    return ModeConfig(
        ...
        min_samples=kwargs.get('min_samples', 10),  # Mode 2阈值更低
        negative_ratio=kwargs.get('negative_ratio', 20),
        tissue_source=kwargs.get('tissue_source', 'Host'),
        exclude_unknown_tissue=kwargs.get('exclude_unknown_tissue', True),
        use_tissue_aware_negatives=kwargs.get('use_tissue_aware_negatives', False),
        use_maml=kwargs.get('use_maml', True),
        inner_lr=kwargs.get('inner_lr', 0.01),
        inner_steps=kwargs.get('inner_steps', 5),
        meta_lr=kwargs.get('meta_lr', 0.001),
        **{k: v for k, v in kwargs.items()
           if k not in ['min_samples', 'negative_ratio', 'tissue_source',
                        'exclude_unknown_tissue', 'use_tissue_aware_negatives',
                        'use_maml', 'inner_lr', 'inner_steps', 'meta_lr']}
    )
...
if __name__ == "__main__":
    ...
    print(f"  Supported strategies: joint, sequential, dynamic")