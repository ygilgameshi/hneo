...
def _stratified_neg_sample(neg_df: pd.DataFrame,
                            pos_sampled: pd.DataFrame,
                            neg_target: int,
                            random_seed: int) -> pd.DataFrame:
    """
    按 (source_protein, peptide_length) 分层采样负样本。

    保证 task balance 过采样后负样本的蛋白分布和长度分布
    与过采样后的正样本严格一致。

    核心逻辑：
      1. 统计过采样后正样本里每个 (source_protein, length) 的比例
      2. 按该比例分配 neg_target 给各组
      3. 各组独立判断 replace，不够就放回采样
      4. 处理舍入误差，保证总量严格等于 neg_target

    Args:
        neg_df:     负样本 DataFrame，须含 source_protein、peptide_length 列
        pos_sampled: 过采样后的正样本 DataFrame，须含 source_protein、peptide_length 列
        neg_target: 目标负样本数
        random_seed: 随机种子

    Returns:
        采样后的 DataFrame（已 reset_index）
    """
    has_cols = ('source_protein' in neg_df.columns and
                'peptide_length' in neg_df.columns and
                'source_protein' in pos_sampled.columns)

    if not has_cols or len(neg_df) == 0:
        # 兼容旧数据，直接随机采样
        replace = len(neg_df) < neg_target
        return neg_df.sample(n=neg_target, replace=replace,
                             random_state=random_seed).reset_index(drop=True)

    # 正样本补 peptide_length 列（若没有）
    if 'peptide_length' not in pos_sampled.columns:
        pos_sampled = pos_sampled.copy()
        pos_sampled['peptide_length'] = pos_sampled['peptide'].str.len()

    # 统计过采样后正样本的 (source_protein, length) 分布
    pos_group_counts = (pos_sampled
                        .groupby(['source_protein', 'peptide_length'])
                        .size())
    total_pos = pos_group_counts.sum()

    parts = []
    for (protein, length), pos_count in pos_group_counts.items():
        # 该组应贡献的负样本数（正比于正样本占比）
        n_needed = max(1, int(neg_target * pos_count / total_pos))

        # 从负样本池中取对应蛋白和长度的子集
        pool = neg_df[
            (neg_df['source_protein'] == protein) &
            (neg_df['peptide_length'] == length)
        ]

        if len(pool) == 0:
            # 该组无对应负样本（蛋白太短切不出来），跳过
            continue

        replace = len(pool) < n_needed
        parts.append(
            pool.sample(n=n_needed, replace=replace, random_state=random_seed)
        )

    if not parts:
        # 极端情况：所有组都没有负样本，退化为随机采样
        replace = len(neg_df) < neg_target
        return neg_df.sample(n=neg_target, replace=replace,
                             random_state=random_seed).reset_index(drop=True)
    ...
    # 处理舍入误差
    if len(result) > neg_target:
        result = result.sample(n=neg_target, random_state=random_seed
                               ).reset_index(drop=True)
    elif len(result) < neg_target:
        shortage = neg_target - len(result)
        replace_fallback = len(neg_df) < shortage
        extra = neg_df.sample(n=shortage, replace=replace_fallback,
...
class AdaptiveTaskBalancedDataset(Dataset):
    ...
    def __init__(self,
        ...
        for task_id, task_df in task_datasets.items():
            ...
            # 采样负样本（按 source_protein 分层，保持蛋白来源分布与正样本一致）
            neg_sampled = _stratified_neg_sample(neg_df, pos_sampled, neg_target, random_seed)
