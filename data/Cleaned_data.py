...
def process_file_with_unique_names(file_path, unique_columns, column_indices, is_first_file=True):
    ...
    df_filtered = df_extracted[
        df_extracted['Species'].astype(str).str.contains('Homo sapiens', case=False, na=False)
    ].copy()

    print(f"过滤Homo sapiens: {len(df_filtered):,} / {before_filter:,} ({len(df_filtered) / before_filter * 100:.1f}%)")
    ...
    # HLA-I标准格式：HLA-A*02:01, HLA-B*07:02, HLA-C*04:01
    hla_pattern = r'^HLA-[ABC]\*\d{2}:\d{2}$'
