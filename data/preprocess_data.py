...
def load_chunked_positives(positive_data_dir='data/cleaned_data', tissue_source='Host'):
    ...
    optional_cols = [
        'Peptide_Length',
        tissue_source, 'Host', 'Inferred_Tissue',
        # 负样本配对采样所需（来自修改后的 Cleaned_data.py）
        'UniProt_ID', 'Epitope_Start', 'Epitope_End',
    ]
...
def prepare_for_mode_system(
    ...
    # UniProt_ID / Epitope_Start / Epitope_End：保留（负样本配对采样需要）
    for col in ['UniProt_ID', 'Epitope_Start', 'Epitope_End']:
    ...
    if add_tissue:
        ...
        if n_tissues == 1 and 'Unknown' in tissue_counts.index:
            ...
            print(f"      --mode mode1")
            raise ValueError("Tissue column is all-Unknown, cannot use Mode 2")
    ...
    # 去重：Mode 2 按 peptide+hla+tissue 去重（同一肽段在不同组织下是独立的正样本）
    #       Mode 1 按 peptide+hla 去重
    if add_tissue and 'tissue' in df.columns:
        dedup_cols = ['peptide', 'hla', 'tissue']
    else:
        dedup_cols = ['peptide', 'hla']

    before = len(df)
    df = df.drop_duplicates(subset=dedup_cols)
    after = len(df)
    if before != after:
        print(f"  ✓ Removed {before-after:,} duplicates (key: {dedup_cols})")
    ...
    # UniProt_ID 覆盖率（用于后续负样本配对采样）
    if 'UniProt_ID' in df.columns:
        n_uniprot = df['UniProt_ID'].notna().sum()
        stats['uniprot_coverage'] = float(n_uniprot / len(df))
        print(f"  UniProt_ID 覆盖: {n_uniprot:,} / {len(df):,} "
              f"({n_uniprot/len(df)*100:.1f}%)  ← 负样本配对采样可用比例")
